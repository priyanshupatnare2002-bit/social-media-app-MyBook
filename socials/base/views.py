...
class ShowProfilePageView(DetailView):
    ...
    def get_context_data(self,*args,**kwargs):
        ...
        if self.request.user.is_authenticated and FollowersCount.objects.filter(
            follower=self.request.user.username, user=page_user.username
        ).exists():
            button_text='UnFollow'
        else:
            button_text='Follow'

        user_followers=len(FollowersCount.objects.filter(user=page_user.username))
        user_following=len(FollowersCount.objects.filter(follower=page_user.username))
